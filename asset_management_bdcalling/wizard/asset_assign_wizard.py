...
class AssetAssignWizard(models.TransientModel):
    ...
    def action_assign(self):
        ...
        # ── 1. Optimistic lock: prevent concurrent assignment ──────────────────
        self.env.cr.execute(
            'SELECT id FROM account_asset WHERE id = %s FOR UPDATE NOWAIT',
            (self.asset_id.id,)
        )
        ...
        if asset.asset_state != 'available':
            raise UserError(_(
                'Asset "%s" must be in "Available" status to assign. '
                'Current status: %s'
