...
class SaleOrderExtended(models.Model):
    ...
    def _clm_move_to_partially_delivered(self):
        """First delivery → PI → Partially Delivered."""
        orders = self.filtered(lambda o: o.clm_state == 'pi')
        if orders:
            orders.with_context(clm_internal_write=True).write({'clm_state': 'partially_delivered'})
            for order in orders:
                order._clm_log_stage_change(
                    from_stage='Proforma Invoice',
                    to_stage='Partially Delivered',
                    trigger='First Delivery Validated',
                )
    
    def _clm_move_to_fully_delivered(self):
        """Final delivery → Partially Delivered → Fully Delivered."""
        orders = self.filtered(lambda o: o.clm_state == 'pi')
        if orders:
            orders.with_context(clm_internal_write=True).write({'clm_state': 'fully_delivered'})
            for order in orders:
                order._clm_log_stage_change(
