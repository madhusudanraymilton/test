...
class HelpdeskApi(http.Controller):
    ...
    @http.route(
        '/api/helpdesk/ticket/<int:ticket_id>/message_history',
        type='http',
        auth='public',
        methods=['GET'],
        csrf=False,
    ...
    def get_message_history(self, ticket_id, **kwargs):
        """Retrieve helpdesk ticket message history"""

        try:
            Ticket = request.env['helpdesk.ticket'].sudo()
            ticket = Ticket.search([('id', '=', ticket_id)], limit=1)
            ...
            for msg in ticket.message_ids.sorted('date'):
